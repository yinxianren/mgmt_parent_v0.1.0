...
/**
 * Excel文件流 --> List <Map<String,Object>>
 * 想直接转成java bean可以使用fastjson将  List<Map<String,Object>>转成bean对象
 *
 */
...
//            log.info(JSON.toJSONString(title));
            ...
            for (int j = 1; j < sheet.getLastRowNum() + 1; j++) {
                ...
                for (int y = row.getFirstCellNum(); y < row.getLastCellNum(); y++) {
                    ...
                    m.put("id", UUID.createShortKey("core_cargo_track"));
                    m.put("userName",UserInfoUtils.getUserName());
                    m.put("currentTime",new Date());
                }
                ls.add(m);
     ...
     * @throws Exception
     */
    public static List<Map<String, Object>> parseToExcel(InputStream in, String fileName,
                                                       Map<String, String> mapping) throws Exception {
        // 根据文件名来创建Excel工作薄
        Workbook work = getWorkbook(in, fileName);
        if (null == work) {
