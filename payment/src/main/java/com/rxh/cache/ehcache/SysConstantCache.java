...
//    @Cacheable(key = "#bankName")
    public SysConstant getBankCodeByBankName(String bankName) {
        return getCache()
                .stream()
                .filter(sysConstant -> StringUtils.equals(bankName,sysConstant.getName())
                        && StringUtils.equals("Bank",sysConstant.getGroupCode())
                )
                .findFirst().orElse(null);
    }
}
