 ...
 */
@Component
public class TransOrderOptimalChannelStrategy extends AbstractMerchantRiskManagement {
    ...
    public  ChannelInfo  strategy(List<ChannelInfo> channelInfoList, TradeObjectSquare tradeObjectSquare) throws PayException {
        ...
        if(null !=riskQuotaDataList && riskQuotaDataList.size() != 0 ) {
            ...
            isNotElement(channelInfoList_day, format("【通道筛选】商户号：%s,订单号：%s：单笔额度超出所有通道的最大单笔额度！", merId, merOrderId));
            //03.获取单月未超额的通道
