...
@Component
public class MerchantsLock extends AbstractPayLock {
     ...
     * @return
     */
    public  synchronized Lock  getLock(String merchantId) throws PayException {
        try{
            //商户号为空，抛出异常
            isNull(merchantId,format("获取商户对象锁时，商户号为 null"));
            Lock lock= merchantsLock.get(merchantId);
            if(null == lock){
                // 查询商户信息
                MerchantInfo merchantInfo = recordPaymentSquareService.getMerchantInfoByMerId(merchantId);
                //如果没该用户，则抛出异常
