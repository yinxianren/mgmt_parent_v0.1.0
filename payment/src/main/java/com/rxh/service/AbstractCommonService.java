...
//                if (!value.matches("[\\w!#$%&'*+/=?^_`{|}~-]+(?:\\.[\\w!#$%&'*+/=?^_`{|}~-]+)*@(?:[\\w](?:[\\w-]*[\\w])?\\.)+[\\w](?:[\\w-]*[\\w])?")) {
     ...
     * @date 20190628
     */
    protected  void setSystemOrderTrack(SystemOrderTrack systemOrderTrack, AbstratorParamModel abstratorParamModel){
     ...
     * @return
     */
    public String errorResult(SystemOrderTrack systemOrderTrack,AbstratorParamModel abstratorParamModel,String message,String resultCode) throws PayException {
        PayTreeMap<String,Object> map= new PayTreeMap<>();
        String merId= systemOrderTrack.getMerId();
        String merOrderId=(null != abstratorParamModel ? abstratorParamModel.getMerOrderId() : "");
        MerchantInfo merchantInfo =redisCacheCommonCompoment.merchantInfoCache.getOne(merId);
