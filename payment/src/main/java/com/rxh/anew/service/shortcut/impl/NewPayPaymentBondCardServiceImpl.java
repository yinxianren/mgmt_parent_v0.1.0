...
//    }
    ...
    @Override
    public MerchantCardTable saveCardInfo(MerchantBondCardApplyDTO mbcaDTO, ChannelInfoTable channelInfoTable, InnerPrintLogObject ipo) throws NewPayException {
        final String localPoint="saveCardInfo";
        MerchantCardTable  merchantCardTable = new MerchantCardTable();
        merchantCardTable.setChannelId(channelInfoTable.getChannelId())
                .setId(System.currentTimeMillis())
                .setOrganizationId(channelInfoTable.getOrganizationId()).setProductId(channelInfoTable.getProductId())
                .setPlatformOrderId("RXH" + new Random(System.currentTimeMillis()).nextInt(1000000) + "-B4" + System.currentTimeMillis())
                .setMerchantId(mbcaDTO.getMerId()).setMerOrderId(mbcaDTO.getMerOrderId()).setTerminalMerId(mbcaDTO.getTerminalMerId())
                .setCardHolderName(mbcaDTO.getCardHolderName())
                .setIdentityType(Integer.valueOf(mbcaDTO.getIdentityType())).setIdentityNum(mbcaDTO.getIdentityNum())
                .setBankCode(mbcaDTO.getBankCode()).setBankCardType(Integer.valueOf(mbcaDTO.getBankCardType()))
                .setBankCardNum(mbcaDTO.getBankCardNum()).setBankCardPhone(mbcaDTO.getBankCardPhone())
                .setValidDate(mbcaDTO.getValidDate()).setSecurityCode(mbcaDTO.getSecurityCode())
        ...
        try {
            ...
            throw new NewPayException(
                    ResponseCodeEnum.RXH99999.getCode(),
                    format("%s-->商户号：%s；终端号：%s；错误信息: %s ；代码所在位置：%s,异常根源：保存绑卡申请信息发生异常,异常信息：%s", ipo.getBussType(), ipo.getMerId(), ipo.getTerMerId(), ResponseCodeEnum.RXH99999.getMsg(), localPoint,e.getMessage()),
                    format(" %s", ResponseCodeEnum.RXH99999.getMsg())
            );
        }
    ...
    public void updateByBondCardInfo(CrossResponseMsgDTO crossResponseMsgDTO, String crossResponseMsg, MerchantCardTable merchantCardTable, InnerPrintLogObject ipo) throws NewPayException {
        ...
        merchantCardTable.setCrossRespResult(crossResponseMsg)
                ...
                .setStatus( crossResponseMsgDTO.getCrossStatusCode() == StatusEnum._0 .getStatus() ? StatusEnum._4.getStatus() : crossResponseMsgDTO.getCrossStatusCode() );
        try {
