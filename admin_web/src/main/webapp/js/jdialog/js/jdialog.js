...
(function ($) {
    ...
    jDialog.prototype.setDialogPostion = function () {
        var scrollTop = (/msie/.test(navigator.userAgent.toLowerCase())) ? document.documentElement.scrollTop : document.body.scrollTop;
        var dialogOffset = $.getDialogsSize() * DIALOG_OFFSET; //窗口位置偏移量
        var _top = scrollTop + ($(window).height() - this.dialog.height()) / 2 + dialogOffset;

        var scrollLeft = (/msie/.test(navigator.userAgent.toLowerCase())) ? document.documentElement.scrollLeft : document.body.scrollLeft;
    ...
    jDialog.prototype.makeDialogContentDiv = function () {
        ...
        var loading = $('<div class="loading"><img src="/img/loading.gif" />正在加载...</div>').appendTo(_contentDiv);
        loading.hide();
    ...
    $.info = function (content, options, width, height) {
        ...
        _content.parent().css("background", "#293846");
    }
